...
class DESApplication(tk.Tk):
    def __init__(self):
        super().__init__()

        self.title("Блочное шифрование - методика DES")
    ...
    def create_widgets(self):
        frame = tk.Frame(self, bg="#f4f2ea")
        frame.pack()

        self.create_button(frame, "Сгенерировать ключ", self.generate_key)
        self.key_label =  self.create_label(frame, text="Ключ:")

        self.create_button(frame, text="Выбрать файл", command=self.select_file)
        self.is_selected = self.create_label(frame, text="")

        self.create_button(frame, text="Зашифровать файл", command=self.encrypt_file)
        encrypted_label = self.create_label(frame, text="Шифр:")
        self.encrypted_label = self.create_label(frame, text="")
        self.encrypted_label.config(wraplength=650)

        self.create_button(frame, text="Дешифровать файл", command=self.decrypt_file)
        decrypted_label = self.create_label(frame, text="Дешифр:")
        self.decrypted_label = self.create_label(frame, text="")
        self.decrypted_label.config(wraplength=650)

    @staticmethod
    def create_button(parent, text, command=None):
        btn = tk.Button(parent, text=text, font=("Roboto", 12, "bold"), bg="#a22581", fg="black",
                        relief="flat", padx=10, pady=5, width=50, height=1, command=command)
        btn.pack(pady=5)

        btn.bind("<Enter>", lambda e: btn.config(bg="#bd66a6"))
        btn.bind("<Leave>", lambda e: btn.config(bg="#a22581"))

    @staticmethod
    def create_label(parent, text):
        label = tk.Label(parent, text=text, font=("Robot", 12, "bold"), fg='black')
        label.pack(pady=5)
        return label

    def generate_key(self, key_size = 4):               # Генерируем 32 битный ключ
        self.key = os.urandom(key_size)
        self.key_label.config(text=f"Ключ: {self.key}")

    def select_file(self):
        file_path = filedialog.askopenfilename(title="Выберите файл", filetypes=[("Binary files", "*.bin"), ("All files", "*.*")])
        if file_path:
            self.selected_file = file_path
            self.is_selected.config(text=f"Выбран файл: {file_path.split("/")[-1]}")
    ...
    def encrypt_file(self):
        ...
        decrypted_blocks = []
        if not self.key:
            messagebox.showerror(title="Ошибка", message="Отсутствует ключ")
        elif not self.selected_file:
            messagebox.showerror(title="Ошибка", message="Не выбран файл для шифрования")
        else:
            for i in range(0, len(self.file_data), 2):          # Делим данные по 16 бит
                des = DES(self.file_data[i:(i + 2)], self.key)
                encrypted_blocks.append(des.encrypted_data)
                decrypted_blocks.append(des.decrypted_data)

            self.encrypted_data = ''.join([str(item) for sublist in encrypted_blocks for item in sublist])
            self.decrypted_data = b''.join(decrypted_blocks)

            self.encrypted_label.config(text=self.encrypted_data)
            new_file_name = self.selected_file[:self.selected_file.find(".bin")] + "_encrypted.bin"
            encrypted_data = list(map(int, self.encrypted_data))
            with open(new_file_name, "wb") as file:
                file.write(DES.bits_to_bytes(encrypted_data))

    def decrypt_file(self):
        self.decrypted_label.config(text=self.decrypted_data.decode('utf-8'))
        new_file_name = self.selected_file[:self.selected_file.find(".bin")] + "_decrypted.bin"
        with open(new_file_name, "wb") as file:
